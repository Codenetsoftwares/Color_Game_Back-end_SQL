...
export const authenticateSuperAdmin = (req, res, next) => {
  ...
  if (!authHeader) {
    return res
      ...
      .send(apiResponseErr(null, false, statusCode.unauthorize, 'Access denied. No token provided.'));
  }
  ...
  jwt.verify(token, process.env.JWT_SECRET_KEY, (err, decoded) => {
    ...
    if (!decoded.roles || !Array.isArray(decoded.roles) || decoded.roles.length === 0) {
      return res
        .status(statusCode.unauthorize)
        .send(apiResponseErr(null, false, statusCode.unauthorize, 'Invalid token or roles not found.'));
    }
