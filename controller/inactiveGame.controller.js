...
export const getInactiveGames = async (req, res) => {
  try {
    ...
    if (markets.length === 0) {
      return res.status(statusCode.success).json(apiResponseSuccess([], true, statusCode.success, 'No markets found'));
    }
    ...
    res.status(statusCode.success).json(
      apiResponseSuccess(paginatedData, true, statusCode.success, 'Success', {
        page: parseInt(page),
